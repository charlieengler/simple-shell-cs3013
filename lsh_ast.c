...
int run_one_program(struct context *context, const struct program *program, struct run_context *run_context, struct argv_buf *argv) {
	...
	if(child_pid == -1) {
		...
		printf("[lsh_ast.c -> run_one_program()] fork error: %d\n", errno);
		...
		// indicated by the third argument being zero. This is run on the parent process.
		...
		// from the argv_buf, *buf. This is run on the child process.
...
int run_pipe_programs(struct context *context, const struct program *program, struct run_context *run_context) {
	...
	struct run_context *lhs_run_context = (struct run_context*)malloc(sizeof(struct run_context));
	lhs_run_context->stdin_fd = STDIN_FILENO;
	lhs_run_context->stdout_fd = pipefd[1];

	struct run_context *rhs_run_context = (struct run_context*)malloc(sizeof(struct run_context));
	rhs_run_context->stdin_fd = pipefd[0];
	rhs_run_context->stdout_fd = STDOUT_FILENO;
	...
	if(child_pid == -1) {
		...
		// indicated by the third argument being zero. This is run on the parent process.
		if(waitpid(child_pid, &rc, 0) != child_pid)
			printf("[lsh_ast.c -> run_pipe_programs()] waitpid error: %d\n", errno);

		// TODO: Error checking and rc might not work
		rc = run_program(context, program->rhs, rhs_run_context);
		...
		// TODO: Error checking and rc might not work
		rc = run_program(context, program->lhs, lhs_run_context);

		// TODO: This may not be right, make sure rc is the correct exit code
		exit(rc);
	}

	free(lhs_run_context);
	free(rhs_run_context);
